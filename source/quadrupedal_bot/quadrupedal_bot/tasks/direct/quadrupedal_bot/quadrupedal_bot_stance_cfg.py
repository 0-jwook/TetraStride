...
class QuadrupedalBotStanceCfg(QuadrupedalBotEnvCfg):
    """Stage 1: 서기 학습 — kp=30 오버댐핑(진동제거), 모든 패널티 처음부터 활성화."""
    ...
    rew_scale_lin_vel_xy: float = -5.0         # X+Y 수평 이동 차단 (버그수정: 기존 Y축만 패널티)
    ...
    rew_scale_stand_still: float = -3.0        # 제자리 유지 강화
    rew_scale_base_drift: float = -5.0         # X+Y 누적 위치 드리프트 패널티 (걷기 방지 핵심)
